...
async function duplicateAndQueueJobs(numberOfDuplicates) {
  ...
  for (let i = 0; i < numberOfDuplicates; i++) {
    ...
    // Collect the message for each set of file keys after copy
    const fileKeys = sourceObjects.map(object => object.Key.replace(sourceFolderName, newFolderKey));
    queueMessages.push({ fileKeys, folderKey: newFolderKey, uniqueId });
  }

  // After all files are copied, send all the messages to the queue
  for (const message of queueMessages) {
    await sendMessageToQueue(queueUrl, message.fileKeys, message.folderKey, message.uniqueId);
  }
}
...
// Call the function with the desired number of duplicates, e.g., 25
duplicateAndQueueJobs(30);
